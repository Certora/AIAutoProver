...
if [[ "${1:-}" == "setup-db" ]]; then
  ...
  # Skip schema init if rag_user already exists — init-db.sql isn't
  # idempotent (plain CREATE USER/DATABASE without IF NOT EXISTS), so
  # blindly re-running it produces noisy errors and partial state.
  if psql -h "$PGHOST" -p "$PGPORT" -U postgres -d postgres -tAc \
    ...
    # Locate AIComposer's bundled init-db.sql via importlib.resources. It
    # ships as package-data of composer.scripts, so pip's install puts it
    # at site-packages/composer/scripts/init-db.sql in this image. The
    # file contains psql \c meta-commands and must go through psql.
